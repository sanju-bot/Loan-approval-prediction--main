...
def predict_from_model(model, scaler, data):
    """Make prediction - 0=Approved, 1=Rejected (alphabetical encoding)"""
    prediction = model.predict(data)
    probability = model.predict_proba(data)
    
    # prediction[0] == 0 means Approved, 1 means Rejected
    is_approved = prediction[0] == 0
    result = "✅ APPROVED" if is_approved else "❌ REJECTED"
    confidence = max(probability[0]) * 100
    
    return result, confidence, int(is_approved)
...
def predict():
    ...
    try:
        ...
        result, confidence, code = predict_from_model(model, scaler, applicant_scaled)
